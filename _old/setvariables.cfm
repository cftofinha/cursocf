...
<br><hr><br>
<cfset DateToday = "Hoje é: #lsDateFormat(now(), 'dd/mm/yyyy')# #lsTimeFormat(now(), 'HH:mm')#">
<cfdump var = "#DateToday#">
...
<cfset DateArray = [
						...
						dateFormat(dateadd('d',1,now()), "short"), 
						dateFormat(dateadd('d',2,now()), "short")]>
<cfdump var = "#DateArray#">

<br><hr><br>
<cfset DateStruct = { 
	today=dateFormat(now(), "short"), 
	tomorrow=dateFormat(dateadd('d',1,now()), "short"), 
	later=dateFormat(dateadd('d',2,now()), "short") }>
